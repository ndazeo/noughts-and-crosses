...
async def get_all_games(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
...
async def create_game(db: Session = Depends(get_db)):
    game = models.Game(finished=False)
    db.add(game)
    db.commit()
    db.refresh(game)
    return game
...
async def make_move(id: str, move: schemas.MoveBase, db: Session = Depends(get_db)):
    game = db.query(models.Game).filter(models.Game.id == id).first()
    if game is None:
        raise HTTPException(status_code=404, detail="Game not found")
    if game.finished:
